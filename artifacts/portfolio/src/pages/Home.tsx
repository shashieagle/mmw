  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-x-hidden">
      ...
      <section className="relative h-screen w-full overflow-hidden bg-black flex items-end pb-20 md:pb-32">
        ...
        {/* Monk — sci-fi digital render */}
        <motion.div
          className="absolute right-0 bottom-0 z-10 w-[48vw] max-w-[680px] pointer-events-none"
          initial={{ opacity: 0, x: 40 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 1.4, delay: 0.4, ease: [0.21, 0.47, 0.32, 0.98] }}
        >
          <div className="relative">
            {/* Base image — B&W high contrast */}
            <img
              src="/monk-hero.png"
              alt="Monk — Business Architect & Storyteller"
              className="w-full h-auto object-contain relative z-10"
              style={{
                filter: "grayscale(1) contrast(1.25) brightness(0.85) drop-shadow(0 0 80px rgba(255,255,255,0.08))",
              }}
            />
            ...
            {/* Circuit SVG overlay */}
            <svg className="absolute inset-0 w-full h-full z-30" viewBox="0 0 640 800" fill="none" xmlns="http://www.w3.org/2000/svg">
              {/* Circuit traces — left side */}
              <motion.path d="M 80 400 L 80 320 L 140 320 L 140 280 L 200 280" stroke="rgba(255,255,255,0.12)" strokeWidth="1" fill="none"
                initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, delay: 1.2 }} />
              <motion.path d="M 60 500 L 60 420 L 120 420 L 120 380 L 180 380" stroke="rgba(255,255,255,0.08)" strokeWidth="1" fill="none"
                initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2.4, delay: 1.5 }} />
              {/* Circuit traces — right side */}
              <motion.path d="M 560 300 L 560 380 L 500 380 L 500 420 L 440 420" stroke="rgba(255,255,255,0.12)" strokeWidth="1" fill="none"
                initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, delay: 1.3 }} />
              <motion.path d="M 580 480 L 580 400 L 520 400 L 520 360" stroke="rgba(255,255,255,0.07)" strokeWidth="1" fill="none"
                initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2.2, delay: 1.6 }} />
              {/* Horizontal data lines */}
              <motion.line x1="40" y1="620" x2="260" y2="620" stroke="rgba(255,255,255,0.06)" strokeWidth="1"
                initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1.5, delay: 1.8 }} style={{ transformOrigin: "left" }} />
              <motion.line x1="380" y1="580" x2="600" y2="580" stroke="rgba(255,255,255,0.06)" strokeWidth="1"
                initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1.5, delay: 2 }} style={{ transformOrigin: "right" }} />
              {/* Node dots */}
              <motion.circle cx="200" cy="280" r="3" fill="rgba(255,255,255,0.3)" initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 2.2 }} />
              <motion.circle cx="140" cy="320" r="2" fill="rgba(255,255,255,0.2)" initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 2.4 }} />
              <motion.circle cx="500" cy="380" r="3" fill="rgba(255,255,255,0.3)" initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 2.3 }} />
              <motion.circle cx="560" cy="300" r="2" fill="rgba(255,255,255,0.2)" initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 2.5 }} />
              {/* Targeting reticle — center */}
              <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8, duration: 0.6 }}>
                <circle cx="320" cy="280" r="28" stroke="rgba(255,255,255,0.12)" strokeWidth="1" fill="none" />
                <line x1="308" y1="280" x2="298" y2="280" stroke="rgba(255,255,255,0.25)" strokeWidth="1" />
                <line x1="332" y1="280" x2="342" y2="280" stroke="rgba(255,255,255,0.25)" strokeWidth="1" />
                <line x1="320" y1="268" x2="320" y2="258" stroke="rgba(255,255,255,0.25)" strokeWidth="1" />
                <line x1="320" y1="292" x2="320" y2="302" stroke="rgba(255,255,255,0.25)" strokeWidth="1" />
              </motion.g>
            </svg>
            ...
            >
              <div className="border border-white/20 bg-black/60 px-3 py-2 backdrop-blur-sm">
                <p className="text-[9px] uppercase tracking-[0.5em] text-white/40 font-bold mb-1">Division 01</p>
                <p className="text-[11px] uppercase tracking-[0.35em] text-white/80 font-bold">MEDIA</p>
                <div className="mt-1.5 flex gap-1">
                  <div className="h-[2px] w-8 bg-white/60" />
                  <div className="h-[2px] w-3 bg-white/20" />
                </div>
              </div>
              <div className="w-px h-12 bg-gradient-to-b from-white/20 to-transparent ml-4" />
            </motion.div>

            {/* BUSINESS ARCHITECTURE label — top right */}
            <motion.div
              className="absolute top-[22%] right-[2%] z-40"
              initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 2.2, duration: 0.8 }}
            >
              <div className="border border-white/20 bg-black/60 px-3 py-2 backdrop-blur-sm">
                <p className="text-[9px] uppercase tracking-[0.5em] text-white/40 font-bold mb-1">Division 02</p>
                <p className="text-[11px] uppercase tracking-[0.35em] text-white/80 font-bold">ARCHITECTS</p>
                <div className="mt-1.5 flex gap-1 justify-end">
                  <div className="h-[2px] w-3 bg-white/20" />
                  <div className="h-[2px] w-8 bg-white/60" />
                </div>
              </div>
              <div className="w-px h-10 bg-gradient-to-b from-white/20 to-transparent ml-auto mr-4" />
            </motion.div>

            {/* Bottom data readout */}
            <motion.div
              className="absolute bottom-44 left-[6%] z-40"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.6, duration: 0.8 }}
            >
              <p className="text-[8px] font-mono text-white/25 tracking-widest">SYS::MONKMONKEYWORKS_v2.0</p>
              <p className="text-[8px] font-mono text-white/15 tracking-widest">NODE_ACTIVE · AI_CORE · ONLINE</p>
            </motion.div>

            {/* Fade out bottom */}
            <div className="absolute bottom-0 left-0 right-0 h-44 bg-gradient-to-t from-black to-transparent z-50" />
            {/* Fade left edge */}
            <div className="absolute inset-y-0 left-0 w-28 bg-gradient-to-r from-black to-transparent z-50" />
          </div>
        </motion.div>
